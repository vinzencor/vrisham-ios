...
interface PaymentVerificationRequest {
  ...
  order_id: string; // Firebase order ID
}

/**
 * Verify Razorpay payment signature and update order status
 */
export const verifyPayment = functions.https.onCall(async (data: PaymentVerificationRequest, context) => {
  ...
  try {
    ...
    if (orderData?.customerID !== context.auth.uid) {
      throw new functions.https.HttpsError(
        "permission-denied",
 ...
 */
export const markPaymentFailed = functions.https.onCall(async (data: { order_id: string; reason?: string }, context) => {
  // Check if user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
